...
char lanip[IP_STR_SIZE] = {0};
char wanip[IP_STR_SIZE] = {0};
char natfile[TMPBUFSIZ] = {0};
char wirepcap[TMPBUFSIZ] = {0};
char wirelesspcap[TMPBUFSIZ] = {0};
...
void outer_callback(u_char *arg, const struct pcap_pkthdr* pkg, const u_char *content)
{
	...
	if(&pos->list == &tstream->list) {
		...
		printf("Create a new stream %s-->%s\n", src_key, dst_key);
...
//pcap_pkthdr包含三个参数：ts时间戳，caplen已捕获部分到长度，len该包到脱机长度
void inner_callback(u_char *arg, const struct pcap_pkthdr* pkg, const u_char *content)
{
	struct pcap_args *args = (struct pcap_args *)arg;
	...
	dprintf("%s %s %ld\n", src_ip, dst_ip, list_node->node.seq);

	if(!memcmp(lanip, src_ip, IP_STR_SIZE)) {
		stream_type = INNER_U;
	} else if(!memcmp(lanip, dst_ip, IP_STR_SIZE)) {
		stream_type = INNER_D;
	} else
	...
	list_for_each_entry_safe(pos, next, &tstream->list, list) {
		...
		if((!memcmp(src_key, pos->lanip_port, IP_PORT_STR_SIZE) \
					&& !memcmp(dst_key, pos->serip_port, IP_PORT_STR_SIZE)) \
				|| (!memcmp(dst_key, pos->lanip_port, IP_PORT_STR_SIZE) \
					&& !memcmp(src_key, pos->serip_port, IP_PORT_STR_SIZE))) {

			dprintf("found a %s stream\n", (stream_type == INNER_U) ? "inner_upload" : "inner_download");
	...
	//all streams had created in outer_callback, so unlikely arrive here
	dprintf("\n");
...
void __caculte_delta(struct pkg_list *orig, struct pkg_list *trans, struct delta_list *delta, struct stream_info *st)
{
	if((NULL == orig) || (NULL == trans) || (NULL == delta))
		return;

	struct pkg_list *p_orig = list_entry(orig->list.next, struct pkg_list, list);
	struct pkg_list *p_trans = list_entry(trans->list.next, struct pkg_list, list);
	struct delta_list *p_delta = delta;

	do {

		if(p_orig->node.seq > p_trans->node.seq) {
			//trans pcap start earlier than orig pcap
			struct pkg_list *pos = p_trans, *next;
			do {
			...
			ptmp->seq = p_orig->node.seq;
			if(p_orig->node.seq == p_trans->node.seq) {
				ptmp->delta_time = (p_trans->node.ts.tv_sec - p_orig->node.ts.tv_sec) * 1000000;
				ptmp->delta_time = ptmp->delta_time + p_trans->node.ts.tv_usec - p_orig->node.ts.tv_usec;
			} else {
				ptmp->delta_time = -10 * 1000000;
			}
			...
			p_trans = list_entry(p_trans->list.next, struct pkg_list, list);
			st->delta_cnts++;
		}

	} while((NULL != p_orig) && (p_orig != orig));
}
...
void caculte_delta()
{
	...
	list_for_each_entry_safe(pos, next, &streams->list, list) {
		...
		__caculte_delta(&pos->streams[INNER_U], &pos->streams[OUTER_U], &pos->delta[0], pos);
		__caculte_delta(&pos->streams[OUTER_D], &pos->streams[INNER_D], &pos->delta[1], pos);
	}
}
...
void usage(char *cmd)
{
	printf("Usage: %s [-l ip] [-g ip] [-f wirepcap] [-n file] [-w wirelesspcap]\n", cmd);
...
{
	struct pcap_pkthdr pkg; //包参数，包括ts时间戳，caplen已捕获部分到长度，len该包到脱机长度
	...
	while((ret = getopt(argc, argv, ":l:f:g:w:h:l")) != -1) {
		switch(ret) {
	...
	iterate_pcap(wirepcap, outer_callback, filter_str, &args);
	...
	if(0 != strlen(wirelesspcap)) {
		...
		iterate_pcap(wirelesspcap, inner_callback, filter_str, &args);
	}
