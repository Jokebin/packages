...
int WriteStreamsToXml()
{
	...
	list_for_each_entry_safe(pos, next, &streams->list, list) {
...
#ifdef DETAIL_INFO //output the detailed info of every stream, not just delta-time
		pkgs = pos->pkgs;
		...
		snprintf(lost_cnts, TMPBUFSIZ, "%d", pos->lost_cnts);
		...
		while(pkgs > 0) {
			...
			xmlNewChild(new_node, NULL, BAD_CAST "Lost", BAD_CAST lost_cnts);
